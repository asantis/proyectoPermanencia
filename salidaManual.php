...
<!-- Confesion: Esta es un "Pagina espejo" del salida.php por que no pude hacer un boton que cambie el cbx a txt osea pude pero la id no podia ser la misma y me arrojaba error... Asi que me fui por lo mas facil e hice este "espejo" que al final sirve por que si la pantalla de salida normal no funciona por las patentes, esta la reemplaza sin problemas -->
...
if(stripos($ua,'android') !== false or stripos($ua,'IPHONE') !== false){
 ...
 <?php 	if(!empty($_POST))
	{
		$alert='';
		if(empty($_POST['patente']) || empty($_POST['operacion']) || empty($_POST['faena']) || empty($_POST['consignatario']) || empty($_POST['nrobl']) || empty($_POST['bulto']) || empty($_POST['isocode']) ||  empty($_POST['cantidad']) || empty($_POST['guia']) || empty($_POST['folio']) || empty($_POST['idingreso']) )
		{
		?><div class="alert">
...
<div class="Registro">
        <center style= "text-align:left";>
           ...
           <a href="salida.php"> <input id="manual" type="button" name="button" value="Modo normal" > </a>
        <hr>
